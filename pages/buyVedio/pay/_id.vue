...
export default {
    ...
    asyncData({
        ...
        return buyVedioApi.createNative(params.id).then(response => {
    ...
    methods: {
        ...
        queryPayStatus(out_trade_no) {
            buyVedioApi.queryPayStatus(out_trade_no)
                .then(response => {
                    if (response.data.code == 25001) {
                        ...
                        //跳转到课程详情页面观看视频
                        this.$router.push({
                            path: "/vediopages/vedioList/" + this.payObj.vedioSortId
                        });
                    }
                });
        }
